---
name: release-notes
description: Generate a prompt for writing a "Notes from the workshop" entry on plankit.com from mcp-bridge's latest CHANGELOG
disable-model-invocation: true
---

Generate a prompt for writing a new "Notes from the workshop" entry on plankit.com for mcp-bridge. Reads mcp-bridge's own `CHANGELOG.md`, distills user-visible changes, and outputs a ready-to-paste prompt for a plankit.com session.

The skill operates on mcp-bridge only. It does not read or write to the plankit.com repo — output is a self-contained prompt the user pastes into a new plankit.com session. Push, not pull.

## Steps

1. Determine the target version:
   - If the user passed a version argument (e.g., `v0.1.0`), use that.
...
3. Distill each commit in the section:
   - **User-visible** — keep: new flags, behavior changes, stderr message changes, new commands, new transport modes, breaking changes.
   ...
   When in doubt, ask: would an mcp-bridge user do something differently tomorrow because of this commit? If no, drop it.
...
**Good example (from a sibling project, for voice calibration):**
...
```
# Write release notes for plankit.com — mcp-bridge <VERSION>

Context: mcp-bridge <VERSION> shipped. Write a new "Notes from the
workshop" entry at `site/mcp-bridge/notes/index.html`. If that file
doesn't exist yet, create it; if it exists, add the new entry at the
top (newest first).
...
read `site/pk/notes/index.html` for the pattern and mirror it.
Breadcrumb should read `plankit / mcp-bridge / notes`. Do not invent
new tokens, new layouts, or new CSS files.

## Link points

After the notes page is in place, add a link from
`site/mcp-bridge/index.html` (footer or a Docs section — match whatever
pattern `site/pk/index.html` uses for its notes link).
...
1. Read `site/pk/notes/index.html` and 1–2 other plankit.com pages to
   absorb style.
2. Draft `site/mcp-bridge/notes/index.html` — show the user before
   writing files.
3. Iterate on voice/length if needed.
4. Add the link point from `site/mcp-bridge/index.html` after the notes
   page is approved.
...
- **Input:** optional version argument (string like `v0.1.0`).
